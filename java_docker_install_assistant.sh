...
if[ $? -eq 0]
	then
		...
		echo "$(tput setaf 10)[Bot assistant]:$(tput setaf 7)  Identifiquei aqui que você não tem nenhuma versão do docker instalado, mas sem problemas, irei resolver isso agora!"
		...
		then
			...
			if [ $VERSAO -eq 20 ]
				then
					...
					sleep 2
					sudo sudo systemctl enable docker -y
